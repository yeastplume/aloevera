...
impl Default for LoggingConfig {
	fn default() -> LoggingConfig {
		LoggingConfig {
			...
			log_file_path: String::from("aloevera.log"),
...
/// This filter is rejecting messages that doesn't start with "aloevera"
/// in order to save log space for only aloevera-related records
#[derive(Debug)]
struct AloeveraFilter;

impl Filter for AloeveraFilter {
	fn filter(&self, record: &Record<'_>) -> Response {
		if let Some(module_path) = record.module_path() {
			if module_path.starts_with("aloevera") {
